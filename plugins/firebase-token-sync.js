(()=>{
  ...
  async function connect(token){const password=String(token||'').trim();if(password.length<12)throw new Error('Use a memorable token of at least 12 characters.');await loadFirebase();lastToken=password;emit('syncing','Connecting…');try{await FAuth.signInWithEmailAndPassword(auth,cfg.loginEmail,password);await startRealtime();return true}catch(error){emit('error',friendly(error));throw new Error(friendly(error))}}
  ...
  async function consumeSetupLink(){const raw=location.hash.startsWith('#')?location.hash.slice(1):'';if(!raw)return;const params=new URLSearchParams(raw),token=params.get('sync');if(!token)return;params.delete('sync');history.replaceState(null,'',location.pathname+location.search+(params.toString()?`#${params}`:''));try{await connect(token)}catch(error){console.warn('Setup-link sync failed',error)}}
  async function boot(){await loadFirebase();if(auth.currentUser)await startRealtime();await consumeSetupLink();setInterval(()=>{if(!auth.currentUser)return;const json=localJSON();if(json!==lastLocal&&json!==lastRemote){lastLocal=json;schedule()}},1000);window.addEventListener('online',()=>auth.currentUser&&startRealtime().catch(()=>{}))}
