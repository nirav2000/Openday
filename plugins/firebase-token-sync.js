(()=>{
  ...
  async function startRealtime(){
    ...
    window.FirebaseUsageMonitor?.listener(1,'state-listener','openday');unsubscribe=FStore.onSnapshot(ref,s=>{window.FirebaseUsageMonitor?.read(1,'state-listener-snapshot','openday');const data=s.data()||{};emitCatalog(data);const remote=data.state;if(remote){const n=normalise(remote),json=JSON.stringify(n);lastRemote=json;const localNow=normalise(readLocal());if(Date.parse(n.updatedAt||0)>Date.parse(localNow.updatedAt||0)){writeLocal(n);lastLocal=json;window.dispatchEvent(new CustomEvent('openday:cloud-state',{detail:n}))}}emit('synced','Synced')},error=>emit('error',friendly(error)));
    emit('synced','Synced');
  }
